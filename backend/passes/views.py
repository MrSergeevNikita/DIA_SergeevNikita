from passes.serializers import *
...
class HighwaysViewSet(viewsets.ModelViewSet):
    queryset = Highways.objects.all()
    serializer_class = HighwaysSerializer
    ...
    def update(self, request, *args, **kwargs):
        ...
        if user_id:
            if User.objects.get(id=user_id).is_superuser:
                ...
                instance.road_length = request.data.get("road_length")
...
class RoadsDepthViewSet(viewsets.ModelViewSet):
    serializer_class = RoadsDepthSerializer
...
class OrdersDepthViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if len(params) > 0:
            if params['id']:
                queryset = Orders.objects.filter(id_customer=params['id'])
        return queryset
